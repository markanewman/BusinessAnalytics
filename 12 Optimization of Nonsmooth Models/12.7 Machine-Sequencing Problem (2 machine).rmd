...
* Minimize: $\sum_{i=1}^{y_{1}}\max(0, \sum_{j=1}^{i}p_{x_{j}} - d_{x_{j}}) + \sum_{i=(y_{1}+1)}^{6}\max(0, \sum_{j=1}^{i}p_{x_{j}} - d_{x_{j}})$
...
M = 2*(max(tab[3,]) + ncol(tab)*(max(tab[3,]) + max(tab[1:2,]) - min(tab[3,])))
fn = function(x) {
...
Domains = t(matrix(c(1,1,1,1,1,1,0,6,6,6,6,6,6,6), nrow = 2, ncol = 7, byrow = TRUE))
sp1 = c(1:6,3)
sp2 = c(6:1,3)
set.seed(1)
